...
function runScript(scriptName) {
  ...
  // Find the script based on OS
  let scriptPath;
  let command;
  
  if (isWindows) {
    // Try PowerShell first (.ps1), fall back to bash via Git Bash
    const psScript = path.join(ROOT_DIR, 'scripts', `${scriptName}.ps1`);
    const bashScript = path.join(ROOT_DIR, 'scripts', `${scriptName}.sh`);
    
    if (path.extname(scriptName) === '.ps1') {
      scriptPath = psScript;
      command = ['powershell', '-ExecutionPolicy', 'Bypass', '-File', scriptPath];
    } else if (path.extname(scriptName) === '.sh' && fs.existsSync(psScript)) {
      // Prefer .ps1 version on Windows
      scriptPath = psScript;
      command = ['powershell', '-ExecutionPolicy', 'Bypass', '-File', scriptPath];
    } else if (fs.existsSync(bashScript)) {
      scriptPath = bashScript;
      // Try Git Bash, fall back to error message
      const whichResult = spawnSync('where', ['gitbash']);
      if (whichResult.status === 0) {
        command = ['C:\\Program Files\\Git\\bin\\bash.exe', '--login', '-c', `"${scriptPath}"`];
      } else {
